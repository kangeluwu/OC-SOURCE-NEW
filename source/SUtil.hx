...
class SUtil
{
	...
	public static function checkFiles():Void
	{
		...
		else if ((FileSystem.exists(SUtil.getPath() + 'windose_data') && !FileSystem.isDirectory(SUtil.getPath() + 'windose_data'))
		...
		else
		{
			...
			{
				Lib.application.window.alert("Whoops, seems like you didn't extract the assets/windose_data folder from the .APK!\nPlease copy the assets/assets folder from the .APK to\n" + SUtil.getPath(),
					'Error!');
				LimeSystem.exit(1);
			}
			else if (FileSystem.exists(SUtil.getPath() + 'windose_data') && !FileSystem.isDirectory(SUtil.getPath() + 'windose_data'))
			{
				Lib.application.window.alert("Bruh why did you create a file called windose_data instead of copying the assets directory from the .APK?, expect a crash.",
					'Error!');
				LimeSystem.exit(1);
			}
			...
			{
				Lib.application.window.alert("Bruh why did you create a file called mods instead of copying the mods directory from the .APK?, expect a crash.",
					'Error!');
				LimeSystem.exit(1);
			}
			
		}
		if (FileSystem.exists('windose_data') && FileSystem.isDirectory('windose_data'))
			{
